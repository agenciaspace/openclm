...
async function loadWorkspace() {
  ...
  const outcome = new URLSearchParams(location.search).get("docusign");
  if (outcome) {
    state.view = "settings";
  ...
  if (outcome) {
    const messages = {
      connected: "DocuSign conectado. Sua integração está pronta.",
...
function shell() {
  ...
  $("#app").innerHTML =
    `<div class="shell"><aside class="sidebar"><a class="brand" href="/" aria-label="OpenCLM início"><img src="/static/icon.svg" alt="">open<em>clm</em></a><div class="side-label">ESPAÇO DE TRABALHO</div><nav class="nav" aria-label="Navegação principal">${menu.map(([key, icon, name]) => `<button data-view="${key}" class="${state.view === key ? "active" : ""}"><span class="nav-icon">${icon}</span>${name}</button>`).join("")}</nav><div class="side-bottom"><div class="privacy-note"><strong>◈ Seus dados ficam com você.</strong>Hospedado na sua infraestrutura.<br>Sem telemetria de uso.</div><div class="profile"><span class="avatar">${escape(state.user.name.slice(0, 2).toUpperCase())}</span><div class="profile-info"><strong>${escape(state.user.name)}</strong><span class="muted small">${roles[state.user.role]}</span></div><button class="icon-button" id="logout" aria-label="Sair da conta" title="Sair">↪</button></div></div></aside><main class="main"><header class="topbar"><div class="breadcrumb">Workspace <span>/ &nbsp; <b id="crumb">Contratos</b></span></div><div class="top-right"><span class="local-badge"><i class="dot"></i> Instância própria</span><a href="/docs" target="_blank" rel="noopener">API ↗</a><button class="icon-button mobile-logout" id="top-logout" aria-label="Encerrar sessão">Sair</button></div></header><div class="content" id="content"></div></main></div>`;
  $$("[data-view]").forEach(
    (button) =>
...
async function showContract(id) {
  const [c, events, versions] = await Promise.all([
    api(`/contracts/${id}`),
